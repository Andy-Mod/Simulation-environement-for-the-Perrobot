...
alpha, beta = calcul_angles(TARGET_HEIGHT, HALF_LEG_LENGTH)
a, b = calcul_angles(0.2, HALF_LEG_LENGTH)

qbut = np.asarray([0.0, a, b])
Xbut = np.asarray(Analogical_MGD(qbut))

#Initialisation de la recherche
qinit = np.asarray([0.0, alpha, beta])
Xinit = np.asarray(Analogical_MGD(qinit))


qfinal = mgi(Xbut, qinit)

print(f"target = {qbut}, obtained={qfinal}")


print("Xinit=", Xinit, type(Xinit))
print("Xbut=", Xbut, type(Xbut))
print("XFinal=", Analogical_MGD(qfinal), type(Analogical_MGD(qfinal)))
